...
export const createReview = async (
	...
	try {
		const { bookId, title, content, rating, imageUrl } = req.body;
		const currentUserId = req.userId!;
...
export const getMyReviews = async (
	...
	try {
		...
		sendSuccessResponse(res, reviews, 'Reviews retrieved successfully');
	} catch (error) {
...
export const updateReview = async (
	...
	try {
		...
		if (!existingReview || existingReview.studentId !== currentUserId) {
			sendErrorResponse(res, 'Review not found or unauthorized', 404);
			return;
		}
